...
def fix_switch_cases(switch_node):
    ...
    """
    assert isinstance(switch_node, vc_ast.Switch)
    if not isinstance(switch_node.stmt, vc_ast.Compound):
        return switch_node

    # The new Compound child for the Switch, which will collect children in the
    # correct order
    new_compound = vc_ast.Compound([], switch_node.stmt.coord)
    ...
    for child in switch_node.stmt.block_items:
        if isinstance(child, (vc_ast.Case, vc_ast.Default)):
...
def _extract_nested_case(case_node, stmts_list):
    ...
    """
    if isinstance(case_node.stmts[0], (vc_ast.Case, vc_ast.Default)):
